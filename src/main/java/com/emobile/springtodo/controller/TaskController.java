...
public class TaskController implements ITaskController {
    ...
    public TaskDto createTask(@RequestBody @Valid NewTaskDto newTaskDto) {
    ...
    @GetMapping
    @ResponseStatus(HttpStatus.OK)
    public List<TaskDto> getTasks(@RequestParam(required = false, defaultValue = "25") int limit,
                                  @RequestParam(required = false, defaultValue = "0") int offset) {
        return TaskMapper.toTaskDtoList(taskService.findAll(limit, offset));
    }

    @PutMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public TaskDto updateTask(@PathVariable int id, @RequestBody @Valid UpdateTaskDto updateTaskDto) {
